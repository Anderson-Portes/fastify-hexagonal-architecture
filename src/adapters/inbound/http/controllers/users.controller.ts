...
export class UsersController {
  ...
  async findAll(request: FastifyRequest, reply: FastifyReply) {
    const users = await this.deps.findAllUsersUseCase.execute()
    return reply.status(200).send(users)
  }

  async findById(request: FastifyRequest, reply: FastifyReply) {
    const { id } = findUserByIdSchema.parse(request.params)
    const user = await this.deps.findUserByIdUseCase.execute(id)
    if (!user) {
  ...
  async create(request: FastifyRequest, reply: FastifyReply) {
    ...
    const user = await this.deps.createUserUseCase.execute(data)
  ...
  async update(request: FastifyRequest, reply: FastifyReply) {
    ...
    const user = await this.deps.updateUserUseCase.execute(id, data)
    return reply.status(200).send(user)
  }

  async delete(request: FastifyRequest, reply: FastifyReply) {
    const { id } = deleteUserParamsSchema.parse(request.params)
    await this.deps.deleteUserUseCase.execute(id)
